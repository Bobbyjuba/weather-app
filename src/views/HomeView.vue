...
const previewCity = (searchResult) => {
  const [city, state] = searchResult.place_name.split(',').map((text) => text.trim());
  router.push({
    name: 'cityView',
    params: {
      state: state,
...
// Move to a secret in GCP
const mapboxApiKey = `pk.eyJ1IjoiYm9iYnlqdWJhIiwiYSI6ImNsbXFyemg3ZjAwajgyaWs0ZGRkbmJxOHcifQ.Tog7nfmpnAmPKpTjvMgTYw`;
